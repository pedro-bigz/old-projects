            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('ava/alunos/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.aluno.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'registro_aluno'">{{ trans('admin.aluno.columns.registro_aluno') }}</th>
                                        <th is='sortable' :column="'data_nascimento'">{{ trans('admin.aluno.columns.data_nascimento') }}</th>
                                        <th is='sortable' :column="'fone'">{{ trans('admin.aluno.columns.fone') }}</th>
                                        <th is='sortable' :column="'email_responsavel'">{{ trans('admin.aluno.columns.email_responsavel') }}</th>
                                        <th is='sortable' :column="'ano_escolar'">{{ trans('admin.aluno.columns.ano_escolar') }}</th>
                                        <th is='sortable' :column="'nivel_escolaridade_id'">{{ trans('admin.aluno.columns.nivel_escolaridade_id') }}</th>
                                ...
                                <tbody>
                                    ...
                                    <td>@{{ item.id }}</td>
                                        <td>@{{ item.registro_aluno }}</td>
                                        <td>@{{ item.data_nascimento | datetime }}</td>
                                        <td>@{{ item.fone }}</td>
                                        <td>@{{ item.email_responsavel }}</td>
                                        <td>@{{ item.ano_escolar }}</td>
                                        <td>@{{ item.nivel_escolaridade_id }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('ava/alunos/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.aluno.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
