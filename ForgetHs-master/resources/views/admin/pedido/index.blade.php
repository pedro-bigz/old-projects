                                ...
                                <div class="row justify-content-md-between">
                                    ...
                                    <div class="switch-filter-wrap">
                                        <label class="switch switch-3d switch-primary">
                                            <input type="checkbox" class="switch-input" v-model="showActiveFilter" >
                                            <span class="switch-slider"></span>
                                        </label>
                                        <span class="label-filter-select">&nbsp;{{ __('Ativos') }}</span>
                                    </div>

                                    <div class="switch-filter-wrap">
                                        <label class="switch switch-3d switch-primary">
                                            <input type="checkbox" class="switch-input" v-model="showParceladoFilter" >
                                            <span class="switch-slider"></span>
                                        </label>
                                        <span class="label-filter-select">&nbsp;{{ __('Parcelados') }}</span>
                                    </div>

                                    <div class="switch-filter-wrap">
                                        <label class="switch switch-3d switch-primary">
                                            <input type="checkbox" class="switch-input" v-model="showAvistaFilter" >
                                            <span class="switch-slider"></span>
                                        </label>
                                        <span class="label-filter-select">&nbsp;{{ __('A Vista') }}</span>
                                    </div>
                                ...
                                <table class="table table-hover table-listing">
                                    <thead>
                                        <tr>
                                            ...
                                            <th is='sortable' :column="'bol_parcelado'">{{ trans('admin.pedido.columns.bol_parcelado') }}</th>
                                            <th is='sortable' :column="'num_parcelas'">{{ trans('admin.pedido.columns.num_parcelas') }}</th>
                                            <th is='sortable' :column="'valor_parcela'">{{ trans('admin.pedido.columns.valor_parcela') }}</th>
                                            <th is='sortable' :column="'valor_total'">{{ trans('admin.pedido.columns.valor_total') }}</th>
                                            {{-- <th is='sortable' :column="'activated'">{{ trans('admin.pedido.columns.activated') }}</th> --}}
                                    ...
                                    <tbody>
                                        <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                            ...
                                            <td>@{{ item.id }}</td>
                                            <td>@{{ item.cliente.user.full_name }}</td>
                                            <td class="text-center" v-if="item.bol_parcelado">
                                                <i class="fa fa-check-square-o" style="color: #20843a"></i>
                                            </td>
                                            <td v-else></td>

                                            <td class="text-center" v-if="item.bol_parcelado">@{{ item.num_parcelas }}</td>
                                            <td v-else></td>

                                            <td class="text-center" v-if="item.bol_parcelado">@{{ item.valor_parcela }}</td>
                                            <td v-else></td>
                                            ...
                                            <td>
                                                <button type="button" class="btn btn-info btn-sm text-white" v-on:click="showDetalhes(item)"><i class="fa fa-plus"></i>&nbsp; Detalhes</button>
                                            </td>
