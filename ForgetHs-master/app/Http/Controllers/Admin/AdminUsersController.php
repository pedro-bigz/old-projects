...
class AdminUsersController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('admin.admin-user.create', [
            ...
            'roles' => Role::where('guard_name', $this->guard)
                ->whereIn('id', [
                    Config::get('constants.options')['ROLE_ADMINISTRADOR'],
                    Config::get('constants.options')['ROLE_MODERADOR']
                ])
                ->get(),
        ]);
    }
    ...
    public function update(UpdateAdminUser $request, AdminUser $adminUser)
    {
        // Sanitize input
        $sanitized = $request->getModifiedData();
        $sanitized['language'] = Config::get('constants.options.DEFAULT_LANGUAGE');

        // if (isset($sanitized['roles'])) {
        //     //VALIDANDO SE NÃO ESTÁ TENTANDO INSERIR ROLE ADMIN
        //     $roleAdmin = Config::get('constants.options')['ROLE_ADMINISTRADOR_NAME'];
        //     foreach ($sanitized['roles'] as $role) {
        //         if(!$this->isAdministrador(Auth::user()) && $role['name'] == $roleAdmin) {
        //             throw new Exception("Você não tem permissão para inserir a role selecionada");
        //         }
        //     }
        // }
    ...
    private function isAdministrador(AdminUser $user) {
