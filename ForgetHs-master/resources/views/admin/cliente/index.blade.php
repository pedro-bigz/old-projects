                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'data_nascimento'">{{ trans('admin.cliente.columns.activated') }}</th>
                                        <th is='sortable' :column="'nivel'">{{ trans('admin.cliente.columns.nivel') }}</th>
                                        <th :column="'nivel'">{{ trans('admin.cliente.columns.historico_pedidos') }}</th>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                        ...
                                        <td>@{{ item.registro_cliente }}</td>
                                        <td>@{{ item.user.full_name }}</td>
                                        <td>@{{ item.telefone }}</td>
                                        <td>@{{ item.celular }}</td>
                                        {{-- <td>@{{ item.user.activated }}</td> --}}
                                        <td>
                                            <label class="switch switch-3d switch-success">
                                                <input type="checkbox" class="switch-input" v-model="collection[index].user.activated" @change="toggleSwitch(item.user.resource_url, 'activated', collection[index].user)">
                                        ...
                                        <td>
                                            ...
                                            <span class="fa fa-star" :class="{'checked_star': item.nivel.id >= 2}"></span>
                                            <span class="fa fa-star" :class="{'checked_star': item.nivel.id >= 3}"></span>
                                            <span class="fa fa-star" :class="{'checked_star': item.nivel.id >= 4}"></span>
                                            <span class="fa fa-star" :class="{'checked_star': item.nivel.id >= 5}"></span>
                                        </td>

                                        <td><a class="btn btn-dark" :href="item.resource_url + '/historico-compras'">Ver histórico</a></td>
